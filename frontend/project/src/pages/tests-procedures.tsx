...
import { DEMO_TESTS, DEMO_PROCEDURES, DEMO_MEDICAL_TERMS } from "@/lib/mock-data"
...
export function TestsDirectoryPage() {
  ...
  const filtered = DEMO_TESTS.filter(test => {
...
export function ProcedureDetailPage() {
  ...
  const { lang } = useLang()
  const navigate = useNavigate()
  const proc = DEMO_PROCEDURES.find(p => p.slug === slug)
