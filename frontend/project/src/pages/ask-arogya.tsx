...
import { DEMO_DOCTORS, DEMO_FACILITIES } from "@/lib/mock-data"
...
export function AskArogyaPage() {
  ...
  const generateResponse = (query: string): Message => {
    ...
    if (q.includes("doctor") || q.includes("cardiolog") || q.includes("physician") || q.includes("डॉक्टर")) {
      const doctors = DEMO_DOCTORS.slice(0, 3)
      return {
        id: crypto.randomUUID(),
        role: "assistant",
        content: lang === "mr"
    ...
    if (q.includes("clinic") || q.includes("open") || q.includes("दवाखाना") || q.includes("उघड")) {
      const facilities = DEMO_FACILITIES.slice(0, 3)
      return {
        id: crypto.randomUUID(),
        role: "assistant",
        content: lang === "mr"
  ...
  const handleSend = (text?: string) => {
    ...
    const assistantMsg = generateResponse(content)
              ...
              <div key={msg.id} className={cn("flex", msg.role === "user" ? "justify-end" : "justify-start")}>
                <div className={cn(
                  ...
                  {msg.structured?.type === "doctor" && Array.isArray(msg.structured.data) && (
                    <div className="mt-3 space-y-2">
                      {(msg.structured.data as typeof DEMO_DOCTORS).map(d => (
                  ...
                  {msg.structured?.type === "facility" && Array.isArray(msg.structured.data) && (
                    <div className="mt-3 space-y-2">
                      {(msg.structured.data as typeof DEMO_FACILITIES).map(f => (
