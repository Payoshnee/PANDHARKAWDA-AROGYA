...
import { DEMO_VERIFICATION_RECORDS, DEMO_VISITING_SESSIONS, DEMO_USER_REPORTS, DEMO_HEALTH_ALERTS, DEMO_DOCTORS } from "@/lib/mock-data"
...
export function AdminDashboardPage() {
  ...
  const pendingVerification = DEMO_VERIFICATION_RECORDS.filter(r => r.status === "pending").length
  const dueForReview = DEMO_DOCTORS.filter(d => isStale(d.last_verified)).length
  const upcomingVisits = DEMO_VISITING_SESSIONS.filter(v => v.status === "confirmed" && new Date(v.visit_date) >= new Date()).length
  const openReports = DEMO_USER_REPORTS.filter(r => r.status === "open").length
  const activeAlerts = DEMO_HEALTH_ALERTS.filter(a => !a.active_until || new Date(a.active_until) >= new Date()).length
    ...
    <div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {DEMO_VISITING_SESSIONS.filter(v => v.status === "confirmed").slice(0, 4).map(v => (
