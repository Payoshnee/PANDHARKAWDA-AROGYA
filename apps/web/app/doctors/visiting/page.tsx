...
export default async function Visiting() {
  ...
  return <div><h1>Visiting Doctors</h1><div className="grid two">{visits.data.length ? visits.data.map((v) => {
    ...
    return <article className="card" key={v.id}><h2>{doctor?.name_en}</h2><p>{doctor?.specialty} at {facility?.name_en}</p><p><strong>{v.visit_date}</strong> · {v.start_time} - {v.end_time}</p><p>{v.booking_info_en}</p><a className="button primary" href={`/doctors/${doctor?.slug}`}>Details</a></article>;
