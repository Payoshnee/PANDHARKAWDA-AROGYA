...
import { DEMO_ADMIN_USERS } from "@/lib/mock-data"
...
import { adminLogin, usePublicData } from "@/lib/api"

export function AdminDoctorsPage() {
  const { lang } = useLang()
  const { data } = usePublicData()
  const [query, setQuery] = useState("")
  const filtered = data.doctors.filter(d => !query || d.name_en.toLowerCase().includes(query.toLowerCase()) || d.name_mr.includes(query))
        ...
        {data.specialties.map(s => (
          <Card key={s.id}>
            <CardContent className="py-4">
              ...
              <p className="text-xs text-muted-foreground mt-1">{data.doctors.filter(d => d.specialty_id === s.id).length} {t("admin.doctors", lang)}</p>
            </CardContent>
          </Card>
        ))}
      </div>
...
export function AdminFacilitiesPage() {
  ...
  const { data } = usePublicData()
  const [query, setQuery] = useState("")
  const filtered = data.facilities.filter(f => !query || f.name_en.toLowerCase().includes(query.toLowerCase()))
...
export function AdminSchedulesPage() {
  const { lang } = useLang()
  const { data } = usePublicData()
  const localDoctors = data.doctors.filter(d => !d.is_visiting)
  return (
    <div>
...
export function AdminFreshnessPage() {
  ...
  const { data } = usePublicData()
  const allEntities = [
    ...data.doctors.map(d => ({ id: d.id, name: getLangValue(d.name_en, d.name_mr, lang), type: "doctor", last_verified: d.last_verified })),
    ...data.facilities.map(f => ({ id: f.id, name: getLangValue(f.name_en, f.name_mr, lang), type: "facility", last_verified: f.last_verified })),
  ]
...
export function AdminInfoPage({ title, type }: { title: string; type: "schemes" | "tests" | "procedures" | "services" | "knowledge" | "health-alerts" }) {
  const { lang } = useLang()
  const { data } = usePublicData()
  const items = type === "schemes" ? data.schemes : type === "tests" ? data.tests : type === "procedures" ? data.procedures : type === "health-alerts" ? data.healthAlerts : []
  return (
    <div>
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((item: { id: string; slug?: string; title_en?: string; title_mr?: string; name_en?: string; name_mr?: string; reviewed_at: string }) => (
...
export function AdminLoginPage() {
  ...
  const [email, setEmail] = useState("admin@arogya.in")
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      toast.error(lang === "mr" ? "लॉगिन अयशस्वी" : "Login failed")
    } finally {
      setLoading(false)
    }
  }
